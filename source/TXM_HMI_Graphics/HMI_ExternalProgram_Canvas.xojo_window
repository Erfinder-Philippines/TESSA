#tag Window
Begin ContainerControl HMI_ExternalProgram_Canvas Implements HMICanvasInterface
...
#tag WindowCode
	...
	#tag Event
		Sub Paint(g As Graphics, areas() As REALbasic.Rect)
		  #Pragma Unused areas
		  If mExternalProgramStep <> Nil Then
		    mExternalProgramStep.Paint(g, 1)
		  End
		End Sub
	#tag EndEvent
	...
	#tag Method, Flags = &h0
		Sub SetHMIClass(HMIClass As HMI_StepClass)
		  // Part of the HMICanvasInterface interface.
		  mExternalProgramStep = ExternalProgram_StepClass(HMIClass)
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub UpdateCanvas(mode As Integer)
		  ...
		  #Pragma Unused mode
		  if mExternalProgramStep <> Nil then
		    HMI_TextArea.Text = mExternalProgramStep.ResultText.GIAS
		    Refresh
		  End
		End Sub
	#tag EndMethod
	...
	#tag Property, Flags = &h21
		Private mExternalProgramStep As ExternalProgram_StepClass = nil
	#tag EndProperty
...
#tag Events HMI_TextArea
	#tag Event
		Sub TextChange()
		  if mExternalProgramStep <> nil then
		    mExternalProgramStep.TextChange(Me.Text)
		  end
		End Sub
	#tag EndEvent
	#tag Event
...
#tag Events Resizer
	#tag Event
		Sub Resize(X As Integer, Y As Integer, Width As Integer, Height As Integer)
		  if mExternalProgramStep <> Nil Then
		    mExternalProgramStep.Resize_HMI_Element(X, Y, Width, Height)
		  End
		End Sub
	#tag EndEvent
#tag EndEvents

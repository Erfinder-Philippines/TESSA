#tag Window
Begin ContainerControl HMI_HxDiagramm_Canvas Implements HMICanvasInterface
...
#tag WindowCode
	#tag Event
		Sub Paint(g As Graphics, areas() As REALbasic.Rect)
		  #Pragma Unused areas
		  if mHxDiagrammStep <> Nil Then
		    mHxDiagrammStep.Paint(g, 1)
		  End
		End Sub
	#tag EndEvent
	...
	#tag Method, Flags = &h0
		Sub SetHMIClass(HMIClass As HMI_StepClass)
		  // Part of the HMICanvasInterface interface.
		  mHxDiagrammStep = HMI_HxDiagramm_StepClass(HMIClass)
		End Sub
	#tag EndMethod
	...
	#tag Property, Flags = &h21
		Private mHxDiagrammStep As HMI_HxDiagramm_StepClass = nil
	#tag EndProperty
...
#tag Events Resizer
	#tag Event
		Sub Resize(X As Integer, Y As Integer, Width As Integer, Height As Integer)
		  if mHxDiagrammStep <> Nil Then
		    mHxDiagrammStep.Resize_HMI_Element(X, Y, Width, Height)
		  End
		End Sub
	#tag EndEvent
#tag EndEvents
